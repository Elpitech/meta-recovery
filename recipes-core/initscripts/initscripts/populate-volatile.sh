...
create_file() {
	...
	[ -e "$1" ] && {
		[ "${DEBUG}" != "no" ] && echo "Target already exists. Skipping."
	} || {
		if [ -z "$ROOT_DIR" ]; then
			eval $EXEC
		else
...
apply_cfgfile() {
	...
	while read TTYPE TUSER TGROUP TMODE TNAME TLTARGET; do
		...
		[ "${DEBUG}" != "no" ] && echo "Checking for -${TNAME}-."

		[ "${TTYPE}" = "l" ] && {
			TSOURCE="$TLTARGET"
			[ "${DEBUG}" != "no" ] && echo "Creating link -${TNAME}- pointing to -${TSOURCE}-."
		...
		[ "${TTYPE}" = "b" ] && {
			TSOURCE="$TLTARGET"
			[ "${DEBUG}" != "no" ] && echo "Creating mount-bind -${TNAME}- from -${TSOURCE}-."
	...
	mount --bind \"${TSOURCE}\" \"${TNAME}\""
		...
		[ -L "${TNAME}" ] && {
			[ "${DEBUG}" != "no" ] && echo "Found link."
			NEWNAME=`ls -l "${TNAME}" | sed -e 's/^.*-> \(.*\)$/\1/'`
			echo ${NEWNAME} | grep -v "^/" >/dev/null && {
				TNAME="`echo ${TNAME} | sed -e 's@\(.*\)/.*@\1@'`/${NEWNAME}"
				[ "${DEBUG}" != "no" ] && echo "Converted relative linktarget to absolute path -${TNAME}-."
			} || {
				TNAME="${NEWNAME}"
				[ "${DEBUG}" != "no" ] && echo "Using absolute link target -${TNAME}-."
			}
		}

		case "${TTYPE}" in
			"f")  [ "${DEBUG}" != "no" ] && echo "Creating file -${TNAME}-."
				TSOURCE="$TLTARGET"
				[ "${TSOURCE}" = "none" ] && TSOURCE=""
				create_file "${TNAME}" "${TSOURCE}" &
				;;
			"d")  [ "${DEBUG}" != "no" ] && echo "Creating directory -${TNAME}-."
				mk_dir "${TNAME}"
				# Add check to see if there's an entry in fstab to mount.
				;;
			*)    [ "${DEBUG}" != "no" ] && echo "Invalid type -${TTYPE}-."
				continue
				;;
		esac
	done
